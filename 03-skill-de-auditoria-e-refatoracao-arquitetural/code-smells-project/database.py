...
def _initialize_db(db_connection):
    ...
    for usuario in usuarios_existentes:
        ...
        if not senha_atual.startswith(("pbkdf2:", "scrypt:", "argon2:")):
            cursor.execute(
                "UPDATE usuarios SET senha = ? WHERE id = ?",
                (generate_password_hash(senha_atual), usuario["id"]),
            )
    db_connection.commit()

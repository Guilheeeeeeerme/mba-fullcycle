from flask import jsonify, request

from database import get_db
import models


VALID_CATEGORIES = ["informatica", "moveis", "vestuario", "geral", "eletronicos", "livros"]
VALID_ORDER_STATUS = ["pendente", "aprovado", "enviado", "entregue", "cancelado"]


def _json_error(message, status_code):
    return jsonify({"erro": message}), status_code


def _internal_error(context, exception):
    print("ERRO " + context + ": " + str(exception))
    return _json_error("Erro interno", 500)


def listar_produtos():
    try:
        produtos = models.get_todos_produtos()
        print("Listando " + str(len(produtos)) + " produtos")
        return jsonify({"dados": produtos, "sucesso": True}), 200
    except Exception as e:
        return _internal_error("ao listar produtos", e)


def buscar_produto(id):
    try:
        produto = models.get_produto_por_id(id)
        if produto:
            return jsonify({"dados": produto, "sucesso": True}), 200
        return _json_error("Produto não encontrado", 404)
    except Exception as e:
        return _internal_error("ao buscar produto", e)


def criar_produto():
    try:
        dados = request.get_json(silent=True) or {}

        if not dados:
            return _json_error("Dados inválidos", 400)
        if "nome" not in dados:
            return _json_error("Nome é obrigatório", 400)
        if "preco" not in dados:
            return _json_error("Preço é obrigatório", 400)
        if "estoque" not in dados:
            return _json_error("Estoque é obrigatório", 400)

        nome = dados["nome"]
        descricao = dados.get("descricao", "")
        preco = dados["preco"]
        estoque = dados["estoque"]
        categoria = dados.get("categoria", "geral")

        if preco < 0:
            return _json_error("Preço não pode ser negativo", 400)
        if estoque < 0:
            return _json_error("Estoque não pode ser negativo", 400)
        if len(nome) < 2:
            return _json_error("Nome muito curto", 400)
        if len(nome) > 200:
            return _json_error("Nome muito longo", 400)
        if categoria not in VALID_CATEGORIES:
            return _json_error("Categoria inválida. Válidas: " + str(VALID_CATEGORIES), 400)

        produto_id = models.criar_produto(nome, descricao, preco, estoque, categoria)
        print("Produto criado com ID: " + str(produto_id))
        return jsonify({"dados": {"id": produto_id}, "sucesso": True, "mensagem": "Produto criado"}), 201
    except Exception as e:
        return _internal_error("ao criar produto", e)


def atualizar_produto(id):
    try:
        dados = request.get_json(silent=True) or {}

        produto_existente = models.get_produto_por_id(id)
        if not produto_existente:
            return _json_error("Produto não encontrado", 404)

        if not dados:
            return _json_error("Dados inválidos", 400)
        if "nome" not in dados:
            return _json_error("Nome é obrigatório", 400)
        if "preco" not in dados:
            return _json_error("Preço é obrigatório", 400)
        if "estoque" not in dados:
            return _json_error("Estoque é obrigatório", 400)

        nome = dados["nome"]
        descricao = dados.get("descricao", "")
        preco = dados["preco"]
        estoque = dados["estoque"]
        categoria = dados.get("categoria", "geral")

        if preco < 0:
            return _json_error("Preço não pode ser negativo", 400)
        if estoque < 0:
            return _json_error("Estoque não pode ser negativo", 400)
        if categoria not in VALID_CATEGORIES:
            return _json_error("Categoria inválida. Válidas: " + str(VALID_CATEGORIES), 400)

        models.atualizar_produto(id, nome, descricao, preco, estoque, categoria)
        return jsonify({"sucesso": True, "mensagem": "Produto atualizado"}), 200
    except Exception as e:
        return _internal_error("ao atualizar produto", e)


def deletar_produto(id):
    try:
        produto = models.get_produto_por_id(id)
        if not produto:
            return _json_error("Produto não encontrado", 404)

        models.deletar_produto(id)
        print("Produto " + str(id) + " deletado")
        return jsonify({"sucesso": True, "mensagem": "Produto deletado"}), 200
    except Exception as e:
        return _internal_error("ao deletar produto", e)


def buscar_produtos():
    try:
        termo = request.args.get("q", "")
        categoria = request.args.get("categoria")
        preco_min = request.args.get("preco_min")
        preco_max = request.args.get("preco_max")

        if preco_min is not None and preco_min != "":
            preco_min = float(preco_min)
        else:
            preco_min = None
        if preco_max is not None and preco_max != "":
            preco_max = float(preco_max)
        else:
            preco_max = None

        resultados = models.buscar_produtos(termo, categoria, preco_min, preco_max)
        return jsonify({"dados": resultados, "total": len(resultados), "sucesso": True}), 200
    except Exception as e:
        return _internal_error("ao buscar produtos", e)


def listar_usuarios():
    try:
        usuarios = models.get_todos_usuarios()
        return jsonify({"dados": usuarios, "sucesso": True}), 200
    except Exception as e:
        return _internal_error("ao listar usuarios", e)


def buscar_usuario(id):
    try:
        usuario = models.get_usuario_por_id(id)
        if usuario:
            return jsonify({"dados": usuario, "sucesso": True}), 200
        return _json_error("Usuário não encontrado", 404)
    except Exception as e:
        return _internal_error("ao buscar usuario", e)


def criar_usuario():
    try:
        dados = request.get_json(silent=True) or {}

        if not dados:
            return _json_error("Dados inválidos", 400)

        nome = dados.get("nome", "")
        email = dados.get("email", "")
        senha = dados.get("senha", "")

        if not nome or not email or not senha:
            return _json_error("Nome, email e senha são obrigatórios", 400)

        usuario_id = models.criar_usuario(nome, email, senha)
        print("Usuário criado: " + email)
        return jsonify({"dados": {"id": usuario_id}, "sucesso": True}), 201
    except Exception as e:
        return _internal_error("ao criar usuario", e)


def login():
    try:
        dados = request.get_json(silent=True) or {}
        email = dados.get("email", "")
        senha = dados.get("senha", "")

        if not email or not senha:
            return _json_error("Email e senha são obrigatórios", 400)

        usuario = models.login_usuario(email, senha)
        if usuario:
            print("Login bem-sucedido: " + email)
            return jsonify({"dados": usuario, "sucesso": True, "mensagem": "Login OK"}), 200

        print("Login falhou: " + email)
        return jsonify({"erro": "Email ou senha inválidos", "sucesso": False}), 401
    except Exception as e:
        return _internal_error("no login", e)


def criar_pedido():
    try:
        dados = request.get_json(silent=True) or {}

        if not dados:
            return _json_error("Dados inválidos", 400)

        usuario_id = dados.get("usuario_id")
        itens = dados.get("itens", [])

        if not usuario_id:
            return _json_error("Usuario ID é obrigatório", 400)
        if not itens:
            return _json_error("Pedido deve ter pelo menos 1 item", 400)

        resultado = models.criar_pedido(usuario_id, itens)

        if "erro" in resultado:
            return jsonify({"erro": resultado["erro"], "sucesso": False}), 400

        print("ENVIANDO EMAIL: Pedido " + str(resultado["pedido_id"]) + " criado para usuario " + str(usuario_id))
        print("ENVIANDO SMS: Seu pedido foi recebido!")
        print("ENVIANDO PUSH: Novo pedido recebido pelo sistema")

        return jsonify(
            {
                "dados": resultado,
                "sucesso": True,
                "mensagem": "Pedido criado com sucesso",
            }
        ), 201
    except Exception as e:
        return _internal_error("ao criar pedido", e)


def listar_pedidos_usuario(usuario_id):
    try:
        pedidos = models.get_pedidos_usuario(usuario_id)
        return jsonify({"dados": pedidos, "sucesso": True}), 200
    except Exception as e:
        return _internal_error("ao listar pedidos do usuario", e)


def listar_todos_pedidos():
    try:
        pedidos = models.get_todos_pedidos()
        return jsonify({"dados": pedidos, "sucesso": True}), 200
    except Exception as e:
        return _internal_error("ao listar pedidos", e)


def atualizar_status_pedido(pedido_id):
    try:
        dados = request.get_json(silent=True) or {}
        novo_status = dados.get("status", "")

        if novo_status not in VALID_ORDER_STATUS:
            return _json_error("Status inválido", 400)

        models.atualizar_status_pedido(pedido_id, novo_status)

        if novo_status == "aprovado":
            print("NOTIFICAÇÃO: Pedido " + str(pedido_id) + " foi aprovado! Preparar envio.")
        if novo_status == "cancelado":
            print("NOTIFICAÇÃO: Pedido " + str(pedido_id) + " cancelado. Devolver estoque.")

        return jsonify({"sucesso": True, "mensagem": "Status atualizado"}), 200
    except Exception as e:
        return _internal_error("ao atualizar status do pedido", e)


def relatorio_vendas():
    try:
        relatorio = models.relatorio_vendas()
        return jsonify({"dados": relatorio, "sucesso": True}), 200
    except Exception as e:
        return _internal_error("no relatorio de vendas", e)


def health_check():
    try:
        db = get_db()
        cursor = db.cursor()
        cursor.execute("SELECT COUNT(*) FROM produtos")
        produtos = cursor.fetchone()[0]
        cursor.execute("SELECT COUNT(*) FROM usuarios")
        usuarios = cursor.fetchone()[0]
        cursor.execute("SELECT COUNT(*) FROM pedidos")
        pedidos = cursor.fetchone()[0]

        return (
            jsonify(
                {
                    "status": "ok",
                    "database": "connected",
                    "counts": {
                        "produtos": produtos,
                        "usuarios": usuarios,
                        "pedidos": pedidos,
                    },
                    "versao": "1.0.0",
                }
            ),
            200,
        )
    except Exception as e:
        return jsonify({"status": "erro", "erro": "Health check indisponível"}), 500
